...
func (r *ReviewRepo) ListByPlace(placeID int) ([]model.Review, error) {
	rows, err := r.db.Query(`
		SELECT rv.id, rv.place_id, rv.food_rating, rv.service_rating, rv.vibe_rating,
			rv.is_gem, rv.comment, rv.image_url, rv.visited_at, rv.created_at, rv.updated_at
		FROM reviews rv
		WHERE rv.place_id = $1
	...
	for rows.Next() {
		...
		if err := rows.Scan(&rv.ID, &rv.PlaceID, &rv.FoodRating, &rv.ServiceRating,
			&rv.VibeRating, &rv.IsGem, &rv.Comment, &rv.ImageURL, &rv.VisitedAt,
...
func (r *ReviewRepo) GetByID(id int) (*model.Review, error) {
	...
	err := r.db.QueryRow(`
		SELECT id, place_id, food_rating, service_rating, vibe_rating,
			is_gem, comment, image_url, visited_at, created_at, updated_at
...
func (r *ReviewRepo) Create(rv *model.Review, authorIDs []int) (*model.Review, error) {
	...
	err = tx.QueryRow(`
		INSERT INTO reviews (place_id, food_rating, service_rating, vibe_rating, is_gem, comment, image_url, visited_at)
		VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
		RETURNING id, created_at, updated_at
	`, rv.PlaceID, rv.FoodRating, rv.ServiceRating, rv.VibeRating,
		rv.IsGem, rv.Comment, rv.ImageURL, rv.VisitedAt,
...
func (r *ReviewRepo) Update(rv *model.Review) (*model.Review, error) {
	_, err := r.db.Exec(`
		UPDATE reviews SET food_rating=$1, service_rating=$2, vibe_rating=$3,
			is_gem=$4, comment=$5, image_url=$6, visited_at=$7, updated_at=now()
		WHERE id=$8
	`, rv.FoodRating, rv.ServiceRating, rv.VibeRating,
		rv.IsGem, rv.Comment, rv.ImageURL, rv.VisitedAt, rv.ID)
	if err != nil {
		return nil, err
	}
	return r.GetByID(rv.ID)
...
func (r *ReviewRepo) getAuthors(reviewID int) ([]model.User, error) {
	rows, err := r.db.Query(`
		SELECT u.id, u.username, u.avatar_url, u.created_at, u.updated_at
		FROM users u
	...
	for rows.Next() {
		var u model.User
		if err := rows.Scan(&u.ID, &u.Username, &u.AvatarURL, &u.CreatedAt, &u.UpdatedAt); err != nil {
			return nil, err
		}
