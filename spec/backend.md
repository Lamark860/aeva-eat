...
### Идентичность места и дубли (реализовано, миграция 016)

Идентичность места — это **физическая точка**, а не имя бренда. Уникальность
определяется индексом `idx_places_identity` по `(LOWER(name), LOWER(COALESCE(address,'')),
LOWER(COALESCE(city,'')))`. Поэтому разные филиалы одной сети (одинаковое имя,
разный адрес) сосуществуют, а точный повтор (то же имя+адрес+город) — ловится.

`POST /api/places` на конфликт идентичности отвечает:
```json
409 { "error": "duplicate", "message": "Такое место уже есть",
      "existing": { /* полная карточка существующего места, как GET /api/places/:id */ } }
```
Фронт по `existing.id` предлагает «перейти и оставить отзыв» (намерение
«отзывы летят на уже добавленное место»), а не показывает тупиковую ошибку.
`PUT /api/places/:id` при переименовании в уже существующее имя+адрес+город
тоже отвечает `409` (раньше была невнятная 500).

> Задел (отложено, см. `TECH-DEBT.md`): хранить Яндекс `org_id` (из `uri`
> геосаджеста) как канонический ключ идентичности и координатный guard для
> ручного ввода. Сущность «бренд» — только когда филиалов станет много.
