...
- ✅ **UX фильтра /places — auto-close + auto-apply на quick-actions.** Раньше я отключил автоматический fetch при клике в дровере (исправляя «прыгает без подтверждения»), но это создало хуже: юзер кликает на «Кто был → lamark», ничего не меняется, drawer открыт, body скролл залочен Bootstrap'ом → впечатление «ничего не работает». Теперь chip «Кто был», date-presets, sort-select — quick-actions: применяются сразу + закрывают drawer + снимают body-lock. Селекты «Город/Кухни/Категории» внутри дровера и date-inputs всё ещё ждут «применить» (там много кликов подряд = ожидаем явный коммит).
