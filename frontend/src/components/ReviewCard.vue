<template>
  <article class="sb-review">
    ...
    <!-- Photo polaroid + gem badge if review.is_gem -->
    <div v-if="review.image_url" class="sb-review-photo">
      <Polaroid
      ...
      >
        ...
        <span v-if="review.is_gem" class="gem-corner">
          <GemBadge :size="22" />
        </span>
      </Polaroid>
    </div>
...
// Per-review tilt / tape variant — deterministic by id so it's stable across renders.
const tilts = ['t-l3', 't-r2', 't-l2', 't-r3']
const photoTilt = computed(() => tilts[(props.review.id ?? 0) % tilts.length])

const tapeVariants = ['', 'rose', 'mint', 'blue']
const tapeVariant = computed(() => tapeVariants[(props.review.id ?? 0) % tapeVariants.length])
...
const tapeStyle = computed(() => {
  ...
  return variants[(props.review.id ?? 0) % variants.length]
})
