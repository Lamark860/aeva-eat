...
### Полировка + фикс nginx-reload (батч 13)
- **Фикс деплоя**: `deploy.yml` теперь перезагружает nginx после `up -d`. Конфиг
  монтируется bind-mount'ом, и `compose up` не перезапускает nginx при изменении
  только содержимого файла → правки nginx-конфига (в т.ч. UUID-регэксп `/p/` из
  батча 12) молча не применялись на проде. reload (zero-downtime) + restart-фолбэк.
