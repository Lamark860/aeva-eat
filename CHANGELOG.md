...
## [Unreleased] — 2026-06-05 — Идентичность места + продуктовый аудит

> Сквозной аудит «MVP → продукт» (полный отчёт — `spec/TECH-DEBT.md`).

### Кэш, корректность, логи (батч 5)
- **Кэш geosuggest** (in-memory, TTL 60s) — повторные запросы автокомплита не
  тратят платную квоту Яндекса.
- **review принадлежит place из URL**: `PUT/DELETE /api/places/:id/reviews/:rid`
  проверяют, что отзыв реально принадлежит указанному месту (404 иначе).
- **Ротация docker-логов** (`json-file`, 10 MB × 3) всем сервисам прод-стека —
  логи не забьют диск общего VPS.

### Данные и оркестрация (батч 4)
- **Канонизация города**: при сохранении город тримится и приводится к уже
  существующему написанию (`CanonicalCity`) — полка «По городам» больше не
  дробится на «Москва»/«москва»/«МОСКВА». Поля места тоже тримятся.
- **Backend healthcheck** в `docker-compose.prod.yml` (`wget /api/health`); nginx
  теперь ждёт `service_healthy` бэка (раньше стартовал раньше готовности → 502).
- Удалён мёртвый компонент `VideoKruzhok.vue`.

### Защита и поиск (батч 3)
- **Rate-limit** (in-memory token-bucket, без новых зависимостей): login/register
  20/мин по IP, suggest-прокси 60/мин по userID → защита от брутфорса паролей и
  слива платной квоты Яндекса (`middleware/ratelimit.go`, 429 + `Retry-After`).
- **Лимит JSON-тела** 1 MB (`MaxBytesReader`, multipart-загрузки не трогает).
- **Поиск по name + city + cuisine** в `/api/places` (раньше — только по
  названию, хотя плейсхолдер обещает «место, кухня, город»).
- **Сохранение `website`**: сайт заведения из Яндекс-подсказки и ручного поля
  теперь пишется (раньше доходил до фронта и терялся).

### Надёжность и безопасность (батч 2)
- **Superuser-bypass на места**: superuser может править/удалять/менять фото
  любого места (общие данные круга), не только свои (`canMutatePlace`).
- **Чистка файлов при удалении места**: обложка места + фото/видео всех отзывов
  удаляются из `/uploads` (раньше оставались сиротами при каскадном DELETE);
  confirm в UI предупреждает, сколько отзывов круга удалится.
- **Graceful degradation карт**: если скрипт Яндекса не загрузился
  (блокировщик/оффлайн/протухший ключ) — показываем сообщение и авто-раскрываем
  ручной ввод адреса, вместо молчаливого пустого блока и мёртвого поиска.
- **Таймауты HTTP-сервера** (`ReadHeaderTimeout`/`IdleTimeout`, защита от
  Slowloris без ограничения заливки видео); **suggest-прокси** с таймаутом 5s и
  контекстом запроса.
- **Видео проверяется по сигнатуре** (magic bytes), а не по присланному
  `Content-Type`; лимит длины (2000) при правке заметки; toast-обработка ошибок
  на удалении места/отзыва и toggle wishlist в `PlaceDetail`.

### Исправлено
- **Нельзя было добавить второй филиал сети** («Волконский в Кремле» при уже
  заведённом «Волконском» в Москве). Причина — уникальный индекс по `(name, city)`
  без адреса. Миграция `016_place_identity` меняет ключ на `(name, address, city)`:
  разные филиалы (разный адрес) теперь создаются нормально. Индекс — строгое
  надмножество старого, на исторических данных не падает; отзывы не трогаются
- **Тупиковый 409 при дубле** — теперь бэк возвращает `409 {error:"duplicate",
  existing:{…}}` с телом существующего места, а форма (`PlaceForm.vue`) показывает
  развилку «перейти и оставить отзыв» / «это другой филиал — уточнить адрес»
  вместо красной плашки
- **Потеря видео/обложки при правке отзыва** — `ReviewRepo.Update` затирал
  `video_url`/`image_url` в NULL при редактировании текста. Теперь Update меняет
  только оценки/коммент/дату; медиа управляется своими эндпоинтами
- **IDOR на удалении инвайта** — `DELETE /api/invites/:id` удалял любой инвайт по
  id. Теперь только создатель или superuser (проверка владения в SQL, 403 иначе)
- Детект конфликта переведён с хрупкого `strings.Contains(err, "idx_…")` на
  типизированный `pq.Error` code `23505`

### Безопасность
- **Fail-fast по `JWT_SECRET`**: в `APP_ENV=production` пустой или дефолтный
  секрет → backend падает на старте (раньше молча подписывал токены публично
  известным `dev-secret-key`). В dev — предупреждение. Добавлена переменная
  `APP_ENV` (dev/prod) в config и оба docker-compose

### Инфраструктура
- `backend/scripts/backup.sh` — бэкап БД + volume `uploads_data` с ротацией;
  cron-инструкция в `DEPLOY.md` (закрывает риск безвозвратной потери контента)
- `spec/TECH-DEBT.md` — реестр известного тех-долга и продуктовых пробелов с
  severity и статусом (актуализация после аудита)
- Документация (`README`, `DEPLOY`, `ROADMAP`, `.env.example`) приведена в
  соответствие с реальностью: полный список API, права на эндпоинты, `APP_ENV`,
  контракт 409, счётчик миграций 001–016

## [0.22.0] — 2026-05-21 — Подготовка к релизу: чистка, секреты, деплой

### Исправлено
- **502 на проде** — `config.DatabaseURL` собирался конкатенацией; пароль БД со
  спецсимволами (`(`) ломал парсинг DSN. Теперь через `url.URL` (URL-экранирование)
- **Дубли в `wishlist_custom`** — `005_seed_data` стоял в авто-миграциях и прогонялся
  при каждом старте, а у таблицы нет уникального индекса → записи плодились
  (×N по числу рестартов). Демо-сид убран из `runMigrations`

### Изменено
- **Секреты вынесены в env**: Яндекс JS API ключ (`VITE_YANDEX_MAPS_KEY`, печётся
  во фронт на сборке) и Геосаджест (`GEOSUGGEST_KEY`) — больше не захардкожены
- **Плавность Доски**: разворот архива анимирует height+opacity вместе, ограниченный
  stagger, `prefers-reduced-motion`
- Демо-хаки ngrok убраны (`hmr` возвращён в dev), `frontend/dist` убран из git

### Инфраструктура
- `DEPLOY.md` — пошаговый деплой на VPS (перенос реальной БД дампом, Traefik/HTTPS)
- `docker-compose.prod.yml` — проброс `GEOSUGGEST_KEY` + build-arg `VITE_YANDEX_MAPS_KEY`
- Репозиторий вычищен (скриншоты, прототипы вёрстки, процессные журналы)
- kill-switch service worker помечен к удалению (~2-3 мес после релиза)
