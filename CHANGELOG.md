...
## [Unreleased] — 2026-06-05…06 — MVP → продукт: аудит + 15 батчей тех-долга

> Сквозной аудит «MVP → продукт» и его устранение в 15 батчах (+ хотфикс).
> Реестр статусов — `spec/TECH-DEBT.md`. Ниже — изменения от свежих к ранним.
> Все critical/high/medium из аудита закрыты и на проде.

### CI nginx-lint (батч 15)
- **CI job `nginx`**: `nginx -t` обоих конфигов (с резолвом upstream) — ловит
  синтаксис до прода. Прямая страховка от инцидента батча 14 (незакавыченный
  регэксп `{36}` уронил nginx).

### Полировка + фикс деплоя nginx (батч 13–14)
- **Фикс деплоя (14)**: `deploy.yml` пересоздаёт nginx (`up -d --force-recreate
  nginx`) после основного `up`. Причина: `nginx.prod.conf` — bind-mount одного
  файла, docker привязывает его по inode на старте; `git reset --hard` заменяет
  файл (новый inode), поэтому ни `up -d`, ни `nginx -s reload`/`restart` не
  подхватывали новый конфиг — только пересоздание контейнера переразрешает mount.
  Без этого UUID-регэксп `/p/` (батч 12) молча не применялся → шаринг по UUID
  не работал на проде. (Батч 13 пробовал reload — недостаточно из-за inode-gotcha.)
- **Унификация копирайта**: все пользовательские «заведение» → «место» (единый
  термин; сущность — `place`, и не всё общепит).
- Единый серверный `/feed` рассмотрен и **осознанно оставлен клиентским**
  (BFF-enrichment) — см. `spec/TECH-DEBT.md` (перенос вёрстки доски в API —
  высокий риск, малая выгода на текущем масштабе).

### Шаринг по UUID (батч 12)
- **`/p/<uuid-token>` вместо `/p/<id>`**: публичные share-страницы шарятся по
  неугадываемому `share_token` (миграция 018, `gen_random_uuid()`), а не по
  инкрементному id — перебором больше не выгрузить каталог названий/городов/обложек.
  nginx-регэксп под UUID (dev+prod), `GetByShareToken`, кнопка «↗ поделиться» в
  карточке места (navigator.share / копирование ссылки).

### Производительность профиля/жемчужин + рефактор (батч 11)
- **`loadPlaces` → `GetManyByIDs`**: страницы профиля и жемчужин больше не зовут
  `GetByID` в цикле (N×8 запросов), а грузят места одним облегчённым набором
  (base + reviewers + feedPhotos + gem_status батч-запросами). Порядок входа
  сохранён, архивированные исключены. detail-only поля (attendance, ratings) на
  карточки не тянутся.
- **Рефактор place-SELECT**: проекция карточки места вынесена в общие
  `placeSelectCols`/`placeBaseFrom`/`scanPlace` — List, GetByID и GetManyByIDs
  используют один источник (устранён дрейф полей между List и GetByID).
- Добавлен интеграционный тест против реального Postgres (`place_db_test.go`,
  пропускается без `AEVA_TEST_DSN` — CI не зависит от БД).

### CI и инфраструктура (батч 10)
- **CI прогоняет миграции против реального Postgres** (новый job `migrations`:
  service-контейнер + boot backend + проверка леджера) — сломанный/не-идемпотентный
  SQL ловится до прода.
- **`environment: production`** в `deploy.yml` — точка для апрува прода (включается
  заданием required reviewers в Settings→Environments).
- **Лимиты mem/cpu** всем сервисам прод-стека (потолки: postgres 768m, backend
  512m, frontend/nginx 128m) — runaway-контейнер не выест RAM общего VPS.

### Миграции с леджером (батч 9)
- **Таблица `schema_migrations`**: миграции больше не прогоняются целиком на
  каждом старте — применяется только новое, каждая в транзакции (SQL + запись
  версии коммитятся вместе → нет частично-применённого состояния, нет
  crash-loop-foot-gun на будущей не-идемпотентной миграции). Проверено на реальном
  Postgres: чистый старт (16 миграций), идемпотентный рестарт (0 повторов),
  prod-сценарий (существующая схема без леджера → разовый идемпотентный перенакат,
  партиал-индексы сохранены).

### Производительность (батч 8)
- **N+1 в `/api/places`**: reviewers и feedPhotos теперь грузятся одним батч-
  запросом каждый (через `= ANY` + оконная функция для per-place LIMIT 5), а не
  циклом по местам. Главный листинг «Найти» из ~1+2N запросов → 3. Форма ответа
  не изменилась. (Профиль/жемчужины через `loadPlaces` — отдельная работа, см.
  `spec/TECH-DEBT.md`.)

### Доступность и корректность (батч 7)
- **a11y подсказок места**: `LocationPicker` получил `role=combobox/listbox/option`,
  `aria-activedescendant`; «Ничего не найдено» теперь валидный `<li>` внутри `<ul>`.
- **TOCTOU лимита фото**: `AddPhoto` блокирует строку отзыва (`FOR UPDATE`) и
  проверяет лимит в транзакции — параллельные загрузки не пробьют 5 фото.

### Продуктовые развилки (батч 6)
- **Мягкое удаление мест**: «удалить» → архив (`deleted_at`). Место исчезает из
  списков/карты/поиска/рандома, но отзывы/фото/видео круга СОХРАНЯЮТСЯ. Superuser
  возвращает через `POST /api/places/:id/restore`. Идентичность места теперь
  партиальная (`WHERE deleted_at IS NULL`) — архив не блокирует повторное добавление.
- **Редактирование общего места — любым участником круга** (по философии «без
  модерации»); удаление в архив — создатель или superuser.
- **Wishlist «исполнение желания» — круговое**: место посетил кто угодно из круга
  → записка общего wishlist зачёркивается у всех (раньше — только у автора визита).
- **CI/CD**: `automerge.yml` переключён со `--squash` на `--rebase` — dev и master
  больше не расходятся, следующий PR не конфликтует.
