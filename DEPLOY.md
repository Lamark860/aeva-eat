...
через леджер `schema_migrations` (каждый файл — максимум один раз, в транзакции;
сами up-скрипты идемпотентны — `IF NOT EXISTS` / `ON CONFLICT`). Демо-данные (`005_seed_data`)
